...
- **SFT**：905,718 条，约 34% 带 reasoning_content、约 9.4% 带工具；统一用数据自带的 chat_template（assistant 永远包 `<think>...</think>`）；AdamW lr=1.5e-4，2 epochs，seq 8192，packing + document-aware mask，NEFTune α=5。**2026-08-15 用户决定**：pretrain 收尾评测后由 `scripts/sft_chain.py` 自动接 SFT（起点新 soup），训练期每 100 步抽样；质量由 7B judge（≥4.0/10）+ think 触发率（≥0.3）+ rep-4（≤0.55）+ 快评裁决，未达标则用 `/media/liuzh/data/DLData/minimind/sft_512.jsonl` 再训 1 epoch；SFT 完成即停止等指令，不自动接 SimPO/RL。
