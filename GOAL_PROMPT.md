...
- **SFT**：905,718 条，约 34% 带 reasoning_content、约 9.4% 带工具；统一用数据自带的 chat_template（assistant 永远包 `<think>...</think>`）；AdamW lr=1.5e-4，2 epochs，seq 8192，packing + document-aware mask，NEFTune α=5。**2026-08-16 用户决定（最新）**：round1（sft_t2t_mini 2 epochs）完成后由 `scripts/sft512_chain.py` **无条件**接 `/media/liuzh/data/DLData/minimind/sft_512.jsonl` 训练 1 epoch（后台 setsid 运行，断网不影响），训练期每 100 步抽样；round2 完成后跑质量报告（7B judge + 快评）并停止等指令，不自动接 SimPO/RL。
